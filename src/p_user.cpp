...
void APlayerPawn::ExecuteActionScript(DWORD buttons, DWORD oldbuttons, int button)
{
	...
	if (script != 0)
	{
		...
		int args[2] = { CLIENT_PREDICT_IsPredicting() ? 1 : 0, wasJustReleased ? 1 : 0 };

		P_StartScript(player->mo, NULL, -script, level.mapname, args, 2, flags);
	}
}
			...
			if (!wasJustThrustedZ || isRampJumper)
			{
				...
				if ( mvFlags & MV_ELEVATORJUMP )
				{
					...
					if ( sector->floordata )
					{
						...
						if ( is3dSector )
						{
							if (sector->floordata->IsKindOf(RUNTIME_CLASS(DDoor)))
							{
								DDoor *door = barrier_cast<DDoor *>(sector->floordata);
								if ( door->GetDirection() == 1 )
									elevatorSpeed = door->GetSpeed();
							}
							else if (sector->floordata->IsKindOf(RUNTIME_CLASS(DCeiling)))
							{
								DCeiling *ceiling = barrier_cast<DCeiling *>(sector->floordata);
...
void P_MovePlayer_Doom(player_t *player, ticcmd_t *cmd)
{
	...
	{
		if ( ( zadmflags & ZADF_NEW_FLY_FORMULA ) || player->bSpectating )
		{
